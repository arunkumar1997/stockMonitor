...
function FundamentalPanel({ fundamentals, valuation }) {
  ...
  const metrics = [
    {
      label: "P/E",
      val: fmt(fundamentals.trailing_pe, "x"),
      tip: "Trailing Price-to-Earnings",
    },
    {
      label: "Fwd P/E",
      val: fmt(fundamentals.forward_pe, "x"),
      tip: "Forward Price-to-Earnings",
    },
    {
      label: "PEG",
      val: fmt(fundamentals.peg_ratio, "", 2),
      tip: "PEG Ratio (<1 = growth at discount)",
    },
    {
      label: "EV/EBITDA",
      val: fmt(fundamentals.ev_to_ebitda, "x"),
      tip: "Enterprise Value / EBITDA",
    },
    {
      label: "P/B",
      val: fmt(fundamentals.price_to_book, "x"),
      tip: "Price-to-Book Ratio",
    },
    {
      label: "D/E",
      val: fmt(fundamentals.debt_to_equity, "", 2),
      tip: "Debt-to-Equity Ratio",
    },
    {
      label: "FCF",
      val: fmtCr(fundamentals.free_cashflow),
      tip: "Free Cash Flow",
    },
    {
      label: "Margin",
      val: fmtPct(fundamentals.profit_margin),
      tip: "Net Profit Margin",
    },
    {
      label: "ROE",
      val: fmtPct(fundamentals.return_on_equity),
      tip: "Return on Equity",
    },
    {
      label: "Rev Grow",
      val: fmtPct(fundamentals.revenue_growth),
      tip: "Revenue Growth YoY",
    },
    {
      label: "Yield",
      val: fmtPct(fundamentals.dividend_yield),
      tip: "Dividend Yield",
    },
  ].filter((m) => m.val !== "—");
    ...
    <Box>
      ...
      {dip.severity !== "none" && (
        <Typography variant="caption" sx={{ color, mt: 0.3, display: "block" }}>
          {dip.severity.charAt(0).toUpperCase() + dip.severity.slice(1)} dip ·
          High: {formatPrice(dip.recent_high)}
        </Typography>
      )}
    </Box>
  );
    ...
    <Box>
      <Box sx={{ display: "flex", justifyContent: "space-between", mb: 0.5 }}>
        ...
        {nearest ? (
          <Typography
            variant="caption"
            sx={{ color: "#00b4d8", fontWeight: 700 }}
          >
...
function StockCard({ stock, onRemove }) {
  ...
  const handleRefresh = useCallback(async () => {
    ...
    timeoutRef.current = setTimeout(() => {
      timeoutRef.current = null;
      if (mountedRef.current) {
        enqueueSnackbar("Refresh timed out — try again", {
          variant: "warning",
        });
        setIsRefreshing(false);
      }
